...
\begin{figure}[t]
  \centering
  % Save this architecture diagram as figures/architecture_multilingual_sae.(pdf|png|jpg)
  \includegraphics[width=\linewidth]{figures/architecture_multilingual_sae}
  \caption{Gemma-2 with Gemma Scope sparse autoencoders attached to residual stream layers (conceptual). 
  Left: example inputs in multiple languages and scripts (EN, HI, UR, BN, TA, TE, DE, AR). 
  Middle: the Gemma-2 2B/9B transformer with SAEs attached at selected layers of the residual stream. 
  Right: each SAE encodes residual activations into 16k sparse features and decodes back, yielding interpretable language, script, syntax, and semantic features from which we construct steering vectors in SAE space.}
  \label{fig:architecture-multilingual}
\end{figure}
...
\begin{algorithm}[t]
  \caption{Constructing an SAE-based steering vector at layer $\ell$}
  \label{alg:steering}
  \begin{algorithmic}[1]
    \REQUIRE Training texts in source language $S$ and target language $T$; SAE at layer $\ell$ with decoder $W_{\text{dec}}$; method $\in$ \{dense, activation-diff, monolinguality, attribution\}; number of features $k$ (for SAE methods).
    \STATE Encode all $S$ and $T$ texts through Gemma to obtain residual activations $h_{\ell,t}$.
    \IF{method = dense}
      \STATE Compute mean activations $\bar{h}_\ell(S)$ and $\bar{h}_\ell(T)$.
      \STATE Set $v_\ell \leftarrow \bar{h}_\ell(T) - \bar{h}_\ell(S)$.
    \ELSE
      \STATE Encode $h_{\ell,t}$ through the SAE encoder to obtain sparse features $z_{\ell,t}$.
      \STATE Estimate activation statistics per feature and language (rates or attribution scores).
      \STATE Select a feature set $S_\ell$ of size $k$ according to the chosen method:
        \STATE \hspace{1.0em}activation-diff: top-$k$ by $\hat{P}(j\mid T) - \hat{P}(j\mid S)$;
        \STATE \hspace{1.0em}monolinguality: top-$k$ by $M_j(T)$;
        \STATE \hspace{1.0em}attribution: top-$k$ by occlusion score $A_{\ell,j}(T)$.
      \STATE Set $v_\ell \leftarrow \frac{1}{k} \sum_{j\in S_\ell} W_{\text{dec}}[:,j]$.
    \ENDIF
    \STATE Normalize $v_\ell$ (e.g., to unit norm) and use it in the forward hook: $h_{\ell,t} \leftarrow h_{\ell,t} + \alpha v_\ell$ during generation.
    \RETURN Steering vector $v_\ell$.
  \end{algorithmic}
\end{algorithm}

\begin{algorithm}[t]
  \caption{Calibrating an LLM-as-judge for steering evaluation}
  \label{alg:calibration}
  \begin{algorithmic}[1]
    \REQUIRE Calibration set of $(\text{prompt}, \text{output})$ pairs with structural ground-truth label $y\in\{0,1\}$ (success/failure); judge predictions $\hat{y}$; desired confidence level.
    \STATE Partition calibration examples into truly-correct ($y=1$) and truly-incorrect ($y=0$).
    \STATE Estimate sensitivity $q_1 \leftarrow P(\hat{y}=1 \mid y=1)$ and specificity $q_0 \leftarrow P(\hat{y}=0 \mid y=0)$ using binomial proportions.
    \STATE For a new evaluation set, compute raw judge-positives rate $p \leftarrow P(\hat{y}=1)$.
    \STATE Compute bias-corrected accuracy $\hat{\theta} \leftarrow (p + q_0 - 1)/(q_0 + q_1 - 1)$ following Lee-style estimators.
    \STATE Construct confidence intervals for $q_0$, $q_1$, and $\hat{\theta}$ using Agresti--Coull or bootstrap methods.
    \RETURN Calibrated judge parameters $(q_0, q_1, \hat{\theta}, \text{CIs})$.
  \end{algorithmic}
\end{algorithm}

\begin{figure}[t]
  \centering
  % Conceptual steering + judge + QA pipeline diagram.
  % Save the final artwork as figures/steering_eval_pipeline.(pdf|png).
  \includegraphics[width=\linewidth]{figures/steering_eval_pipeline}
  \caption{Steering, evaluation, and calibrated judge pipeline (conceptual). Steering vectors (dense, SAE activation-difference, monolinguality, attribution) are applied at a chosen layer in Gemma-2. We compare baseline vs.\ steered generations using structural metrics (script ratios, LaBSE similarity, repetition/degradation) and a Gemini-based LLM judge with Lee-style calibration to obtain bias-corrected accuracy estimates, while also measuring QA performance on MLQA and IndicQA.}
...
\begin{figure*}[t]
  \centering
  % Save the overview comic as figures/overview_comic.(pdf|png|jpg)
  \includegraphics[width=\textwidth]{figures/overview_comic}
  \caption{How we steer Gemma's multilingual brain (conceptual). 
  Panel~1: multilingual prompts in English and Indic languages. 
  Panel~2: Gemma-2 with SAEs at multiple layers; the residual stream is decomposed into sparse features and a mid-layer shared concept space, while late layers host language-specific decoding lobes. 
  Panel~3: changing the steering strength $\alpha$ moves generations from English outputs to Indic outputs. 
  Panel~4: we evaluate script choice, semantic similarity (LaBSE), degradation, and QA performance, and calibrate a Gemini-based LLM judge.}
  \label{fig:overview}
\end{figure*}

% Placeholder empirical plot from plots.py (will be regenerated and cleaned for the final draft).
\begin{figure}[t]
  \centering
  % This file is produced by plots.py as part of the automatic summary.
  \includegraphics[width=\linewidth]{results/figures/fig1_feature_counts_by_layer}
  \caption{Placeholder empirical plot (auto-generated): number of strongly language-selective SAE features (monolinguality $M>3$) per layer in Gemma-2B. The final version of the paper will replace this placeholder with a cleaned version of the same plot generated from the latest experimental run.}
  \label{fig:feature-counts-placeholder}
\end{figure}

\subsection{Geometry of Indic Representations (Exp1, Exp3, Exp5, Exp6)}

\paragraph{Exp1: Feature Discovery (Monolinguality Detectors).}
...
\paragraph{Exp3: Hindi--Urdu Overlap (Correct Jaccard).}
...
\paragraph{Exp5 \& Exp6: Hierarchical and Script vs Semantic Structure.}
...
\begin{figure*}[t]
  \centering
  % Save this conceptual diagram as figures/language_concept_space.(pdf|png|jpg)
  \includegraphics[width=\textwidth]{figures/language_concept_space}
  \caption{Language-agnostic concept space across depth (conceptual). 
  This schematic is \emph{not} a substitute for empirical plots; it is a visual summary of the qualitative patterns observed in our experiments. 
  Left: early layers are dominated by orthography and tokenization; languages cluster loosely by script, consistent with early-layer feature overlaps and script controls (Exp5, Exp6). 
  Middle: mid layers form a shared semantic concept space where Indic and non-Indic languages overlap strongly; Hindi--Urdu form a tight cluster, reflecting their consistently higher Jaccard overlap compared to Hindi--English (Exp3) and cross-layer alignment (Exp14). 
  Right: late layers specialize for language-specific decoding and script, adding a shell of language- and script-specific detectors on top of the shared core while preserving strong HI--UR similarity, in line with late-layer detector counts (Exp1) and script-specific fractions (Exp3, Exp6).}
  \label{fig:concept-space}
\end{figure*}

\subsection{Steering Methods and Layers (Exp2, Exp7, Exp9, Exp10)}

\paragraph{Exp2: Steering method comparison at selected layers.}
\textbf{Goal.} Compare dense, activation-diff SAE, monolinguality SAE, and random steering at a small set of layers (e.g., 5, 13, 20, 24) for EN$\to$HI steering.

\textbf{Method.} We construct steering vectors for each method using the procedure in Algorithm~\ref{alg:steering} and apply them at the chosen layers while generating from FLORES EN prompts.

\textbf{Results (partial).} In an initial EN$\to$HI steering run on FLORES prompts:
\begin{itemize}
...
\paragraph{Exp7: Single-feature causal probing.}
\textbf{Goal.} Estimate per-feature generator scores $G_{\ell,j}(T)$ and attribution scores $A_{\ell,j}(T)$ for selected SAEs.

\textbf{Method.} For a set of candidate features at layers of interest, we perform single-feature steering per \eqref{eq:single-feature-steer} and occlusion per \eqref{eq:attribution-score}, measuring script-based success differences on a small prompt set.

\textbf{Results.} \textbf{N/A} (exploratory; experiment to be fully re-run).

\paragraph{Exp9: Full layer $\times$ method $\times$ language sweep.}
\textbf{Goal.} Systematically evaluate steering success and degradation across layers, methods, and target languages.

\textbf{Method.} For each layer in $\{5,8,10,13,16,20,24\}$, each steering method (dense, activation-diff SAE, monolinguality SAE, random), and each target language (HI, BN, TA, TE, UR, DE, AR), we build a steering vector via Algorithm~\ref{alg:steering}, generate from multiple prompts at several strengths, and aggregate script, semantic, degradation, and LLM-judge metrics.

\textbf{Results.} \textbf{N/A} (planned main sweep; will be populated from \texttt{exp9\_layer\_sweep\_steering.json} and plots).

\paragraph{Exp10: Attribution-based steering.}
\textbf{Goal.} Use occlusion attribution to select causally important features and construct attribution-based steering vectors.

\textbf{Method.} We use occlusion scores $A_{\ell,j}(T)$ to choose top-$k$ features and build steering vectors via Algorithm~\ref{alg:steering}, then compare their performance to dense and activation-diff steering.

\textbf{Results.} \textbf{N/A} (exploratory; will be populated from \texttt{exp10\_attribution\_occlusion.json}).

\subsection{Spillover and Scaling (Exp4, Exp8)}

\paragraph{Exp4: Spillover analysis.}
\textbf{Goal.} Measure how EN$\to$Hindi steering affects other Indic languages vs non-Indic controls.

\textbf{Method.} Apply a Hindi steering vector at selected layers and classify outputs by script/language, measuring probabilities of outputs in HI, UR, BN, TA, TE, DE, AR (and optionally EN$\to$DE as a control steering direction).

\textbf{Results.} \textbf{N/A} (will be populated from \texttt{exp4\_spillover.json} and associated plots).

\paragraph{Exp8: Scaling to 9B and low-resource languages.}
...
\textbf{Method.} Compute active feature counts per language and layer and evaluate dense vs SAE steering on a small set of prompts for EN$\to$HI, treating 9B as a sanity check rather than a full scaling study.

\textbf{Results.} \textbf{N/A} (sanity-check experiment; will be populated from \texttt{exp8\_scaling\_9b\_low\_resource.json}).

\subsection{Judge Calibration and QA Degradation (Exp11, Exp12)}

\paragraph{Exp11: Calibrated LLM-as-judge.}
\textbf{Goal.} Calibrate a Gemini-based judge for multilingual steering evaluation.

\textbf{Method.} Use a calibration set with structural ground-truth labels to estimate sensitivity, specificity, and bias-corrected accuracy with confidence intervals as in recent LLM-as-judge analyses~\cite{krumdick2025nofreelabels}.

\textbf{Results.} \textbf{N/A} (to be filled from \texttt{exp11\_judge\_calibration.json} and calibration plots).

\paragraph{Exp12: QA degradation under steering.}
\textbf{Goal.} Quantify how EN$\to$target steering affects QA performance on MLQA and IndicQA.

\textbf{Method.} For each target language (HI, DE, AR; HI, BN, TA, TE), we use Exp9 results to pick a steering configuration, generate baseline and steered answers for MLQA/IndicQA questions, and evaluate script dominance, LaBSE similarity to gold answers, degradation, and LLM-judge scores.

\textbf{Results.} \textbf{N/A} (to be filled from \texttt{exp12\_qa\_degradation.json} and QA metric plots).
